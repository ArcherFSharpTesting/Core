module Archer.Arrows.Tests.Feature.Test.TestBodyIndicator.Teardown.``087 - Feature Test with test body indicator two parameters, teardown should``
...
let ``Create a valid ITest`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, test), testName, (path, fileName, lineNumber) =
            TestBuilder.BuildTestWithTestBodyTwoParametersTearDown testFeature
