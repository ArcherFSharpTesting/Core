module Archer.Arrows.Tests.Feature.Test.Tags.Setup.Data.TestBodyIndicator.``050 - Feature Test with tags, setup, data, test body indicator two parameters should``
...
let ``Create a valid ITest`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (tags, _, data, testNameRoot), (path, fileName, lineNumber) =
            TestBuilder.BuildTestWithTagsSetupDataTestBodyTwoParametersNameHints testFeature
...
let ``Create a test name with name hints and repeating data`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (_, _, data, testNameRoot), _ =
            TestBuilder.BuildTestWithTagsSetupDataTestBodyTwoParametersNameHints (testFeature, true)
...
let ``Create a test name with no name hints`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (_, _, data, testName), _ =
            TestBuilder.BuildTestWithTagsSetupDataTestBodyTwoParameters testFeature
...
let ``Create a test name with no name hints same data repeated`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (_, _, data, testName), _ = TestBuilder.BuildTestWithTagsSetupDataTestBodyTwoParameters (testFeature, true)
...
let ``Call setup when executed`` =
    feature.Test (fun (featureSetupValue, testFeature: IFeature<string>) ->
        let (monitor, tests), _, _ = TestBuilder.BuildTestWithTagsSetupDataTestBodyTwoParameters testFeature
...
let ``Call Test when executed`` =
    feature.Test (fun (featureSetupValue, testFeature: IFeature<string>) ->
        let (monitor, tests), (_, setupValue, data, _), _ = TestBuilder.BuildTestWithTagsSetupDataTestBodyTwoParameters testFeature
