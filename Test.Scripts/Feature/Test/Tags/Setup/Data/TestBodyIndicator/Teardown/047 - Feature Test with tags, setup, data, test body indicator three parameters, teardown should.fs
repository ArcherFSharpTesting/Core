module Archer.Arrows.Tests.Feature.Test.Tags.Setup.Data.TestBodyIndicator.Teardown.``047 - Feature Test with tags, setup, data, test body indicator three parameters, teardown should``
...
let ``Create a valid ITest`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
       let (_, tests), (tags, _, data, testNameRoot), (path, fileName, lineNumber) =
           TestBuilder.BuildTestWithTagsSetupDataTestBodyThreeParametersTeardownNameHints testFeature
...
let ``Create a test name with name hints and repeating data`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
       let (_, tests), (_, _, data, testNameRoot), _ =
           TestBuilder.BuildTestWithTagsSetupDataTestBodyThreeParametersTeardownNameHints (testFeature, true)
...
let ``Create a test name with no name hints`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
       let (_, tests), (_, _, data, testName), _ =
           TestBuilder.BuildTestWithTagsSetupDataTestBodyThreeParametersTeardown testFeature
...
let ``Create a test name with no name hints same data repeated`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
       let (_, tests), (_, _, data, testName), _ =
           TestBuilder.BuildTestWithTagsSetupDataTestBodyThreeParametersTeardown (testFeature, true)
...
let ``Call setup when executed`` =
    feature.Test (fun (featureSetupValue, testFeature: IFeature<string>) ->
       let (monitor, tests), _, _ = TestBuilder.BuildTestWithTagsSetupDataTestBodyThreeParametersTeardown testFeature
...
let ``Call Test when executed`` =
    feature.Test (fun (featureSetupValue, testFeature: IFeature<string>) ->
        let (monitor, tests), (_, setupValue, data, _), _ = TestBuilder.BuildTestWithTagsSetupDataTestBodyThreeParametersTeardown testFeature
