module Archer.Arrows.Tests.Feature.Test.Tags.Data.TestBodyIndicator.``058 - Feature Test with tags, data, test body indicator two three parameters should``
...
let ``Create a valid ITest`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (tags, data, testNameRoot), (path, fileName, lineNumber) =
            TestBuilder.BuildTestWithTagsDataTestBodyThreeParametersNameHints testFeature
...
let ``Create a test name with name hints and repeating data`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (_, data, testNameRoot), _ =
            TestBuilder.BuildTestWithTagsDataTestBodyThreeParametersNameHints (testFeature, true)
...
let ``Create a test name with no name hints`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (_, data, testName), _ =
            TestBuilder.BuildTestWithTagsDataTestBodyThreeParameters testFeature
...
let ``Create a test name with no name hints same data repeated`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (_, data, testName), _ =
            TestBuilder.BuildTestWithTagsDataTestBodyThreeParameters (testFeature, true)
...
let ``Call setup when executed`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (monitor, tests), _, _ = TestBuilder.BuildTestWithTagsDataTestBodyThreeParameters testFeature
...
let ``Call Test when executed`` =
    feature.Test (fun (featureSetupValue, testFeature: IFeature<string>) ->
        let (monitor, tests), (_, data, _), _ = TestBuilder.BuildTestWithTagsDataTestBodyThreeParameters testFeature
