module Archer.Arrows.Tests.Feature.Test.TestName.Data.TestBodyIndicator.``036 - Feature Test with test name, data, test body indicator two parameters should``
...
let ``Create a valid ITest`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (data, testNameRoot), (path, fileName, lineNumber) =
            TestBuilder.BuildTestWithTestNameDataTestBodyTwoParametersNameHints testFeature
...
let ``Create a test name with name hints and repeating data`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (data, testNameRoot), _ =
            TestBuilder.BuildTestWithTestNameDataTestBodyTwoParametersNameHints (testFeature, true)
...
let ``Create a test name with no name hints`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (data, testName), _ =
            TestBuilder.BuildTestWithTestNameDataTestBodyTwoParameters testFeature
...
let ``Create a test name with no name hints same data repeated`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (data, testName), _ =
            TestBuilder.BuildTestWithTestNameDataTestBodyTwoParameters (testFeature, true)
...
let ``Not call setup when executed`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (monitor, tests), _, _ = TestBuilder.BuildTestWithTestNameDataTestBodyTwoParameters testFeature
...
let ``Call Test when executed`` =
    feature.Test (fun (featureSetupValue, testFeature: IFeature<string>) ->
        let (monitor, tests), (data, _), _ = TestBuilder.BuildTestWithTestNameDataTestBodyTwoParameters testFeature
