module Archer.Core.Tests.Feature.Test.TestName.Tags.Setup.Data.TestBodyIndicator.``003 - Feature Test with test name, tags, setup, data, test body indicator three parameters should``
...
let private feature = FeatureBuilder.eBuilder.eBuilder.eBuilder.NewFeature (
    TestTags [
        Category "Feature"
        Category "Test"
    ],
...
let ``Create a test name with no name hints same data repeated`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (_, _, data, testName), _ = TestBuilder.BuildTestWithTestNameTagsSetupDataTestBodyThreeParameters (testFeature, true)
