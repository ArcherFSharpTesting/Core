module Archer.Arrows.Tests.Feature.Test.TestName.Setup.Data.TestBodyIndicator.``026 - Feature Test with test name, setup, data, test body indicator three parameters``
...
let ``Create a valid ITest`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (_, data, testNameRoot), (path, fileName, lineNumber) =
            TestBuilder.BuildTestWithTestNameSetupDataTestBodyThreeParametersNameHints testFeature
...
let ``Create a test name with name hints and repeating data`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (_, data, testNameRoot), _ =
            TestBuilder.BuildTestWithTestNameSetupDataTestBodyThreeParametersNameHints (testFeature, true)
...
let ``Create a test name with no name hints`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (_, data, testName), _ =
            TestBuilder.BuildTestWithTestNameSetupDataTestBodyThreeParameters testFeature
...
let ``Create a test name with no name hints same data repeated`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (_, data, testName), _ =
            TestBuilder.BuildTestWithTestNameSetupDataTestBodyThreeParameters (testFeature, true)
...
let ``Call Test when executed`` =
    feature.Test (fun (featureSetupValue, testFeature: IFeature<string>) ->
        let (monitor, tests), (setupValue, data, _), _ = TestBuilder.BuildTestWithTestNameSetupDataTestBodyThreeParameters testFeature
