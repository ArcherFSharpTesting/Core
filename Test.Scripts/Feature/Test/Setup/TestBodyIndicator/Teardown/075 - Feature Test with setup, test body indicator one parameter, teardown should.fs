module Archer.Arrows.Tests.Feature.Test.Setup.TestBodyIndicator.Teardown.``075 - Feature Test with setup, test body indicator one parameter, teardown should``
...
let ``Create a valid ITest`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, test), (_, testName), (path, fileName, lineNumber) =
            TestBuilder.BuildTestWithSetupTestBodyOneParameterTeardown testFeature
...
let ``Call setup when executed`` =
    feature.Test (fun (featureSetupValue, testFeature: IFeature<string>) ->
        let (monitor, test), _, _ = TestBuilder.BuildTestWithSetupTestBodyOneParameterTeardown testFeature
...
let ``Call Test when executed`` =
    feature.Test (fun (featureSetupValue, testFeature: IFeature<string>) ->
        let (monitor, test), (testSetupValue, _), _ = TestBuilder.BuildTestWithSetupTestBodyOneParameterTeardown testFeature
