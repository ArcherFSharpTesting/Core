module Archer.Arrows.Tests.Feature.Test.Setup.Data.TestBodyIndicator.Teardown.``070 - Feature Test with setup, data, test body indicator three parameters, teardown``
...
let ``Create a valid ITest`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (_, data, testNameRoot), (path, fileName, lineNumber) =
            TestBuilder.BuildTestWithSetupDataTestBodyThreeParametersTeardownNameHints testFeature
...
let ``Create a test name with name hints and repeating data`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (_, data, testNameRoot), _ =
            TestBuilder.BuildTestWithSetupDataTestBodyThreeParametersTeardownNameHints (testFeature, true)
...
let ``Create a test name with no name hints`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (_, data, testName), _ =
            TestBuilder.BuildTestWithSetupDataTestBodyThreeParametersTeardown testFeature
...
let ``Create a test name with no name hints same data repeated`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (_, data, testName), _ =
            TestBuilder.BuildTestWithSetupDataTestBodyThreeParametersTeardown (testFeature, true)
...
let ``Call setup when executed`` =
    feature.Test (fun (featureSetupValue, testFeature: IFeature<string>) ->
        let (monitor, tests), _, _ = TestBuilder.BuildTestWithSetupDataTestBodyThreeParametersTeardown testFeature
...
let ``Call Test when executed`` =
    feature.Test (fun (featureSetupValue, testFeature: IFeature<string>) ->
        let (monitor, tests), (testSetupValue, data, _), _ = TestBuilder.BuildTestWithSetupDataTestBodyThreeParametersTeardown testFeature
