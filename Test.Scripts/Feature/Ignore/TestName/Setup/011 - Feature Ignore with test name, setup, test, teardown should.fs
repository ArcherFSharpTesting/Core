module Archer.Arrows.Tests.Feature.Ignore.TestName.Setup.``011 - Feature Ignore with test name, setup, test, teardown should``
...
let ``Create a valid ITest`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, test), testName, (path, fileName, lineNumber) =
            IgnoreBuilder.BuildTestWithTestNameSetupTestBodyTeardown testFeature
...
let ``Not call setup when executed`` =
    feature.Test (fun (featureSetupValue, testFeature: IFeature<string>) ->
        let (monitor, test), _, _ = IgnoreBuilder.BuildTestWithTestNameSetupTestBodyTeardown testFeature
