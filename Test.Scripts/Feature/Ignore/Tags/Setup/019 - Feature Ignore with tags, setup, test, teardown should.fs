module Archer.Arrows.Tests.Feature.Ignore.Tags.Setup.``019 - Feature Ignore with tags, setup, test, teardown should``
...
let ``Create a valid ITest`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
       let (_, test), (tags, testName), (path, fileName, lineNumber) =
           IgnoreBuilder.BuildTestWithTagsSetupTestBodyTeardown testFeature
...
let ``Not call setup when executed`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
       let (monitor, test), _, _ = IgnoreBuilder.BuildTestWithTagsSetupTestBodyTeardown testFeature
