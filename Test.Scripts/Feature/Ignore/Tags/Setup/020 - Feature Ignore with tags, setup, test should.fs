module Archer.Arrows.Tests.Feature.Ignore.Tags.Setup.``020 - Feature Ignore with tags, setup, test should``
...
let ``Create a valid ITest`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
       let (_, test), (tags, testName), (path, fileName, lineNumber) =
           IgnoreBuilder.BuildTestWithTagsSetupTestBody testFeature
...
let ``Not call setup when executed`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
       let (monitor, test), _, _ = IgnoreBuilder.BuildTestWithTagsSetupTestBody testFeature
...
let ``Not call Test when executed`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
       let (monitor, test), (_, _), _ = IgnoreBuilder.BuildTestWithTagsSetupTestBody testFeature
