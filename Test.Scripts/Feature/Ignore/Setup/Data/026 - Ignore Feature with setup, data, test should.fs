module Archer.Arrows.Tests.Feature.Ignore.Setup.Data.``026 - Ignore Feature with setup, data, test should``
...
let ``Create a valid ITest`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (data, testNameRoot), (path, fileName, lineNumber) =
            IgnoreBuilder.BuildTestWithSetupDataTestBodyNameHints testFeature
...
let ``Create a test name with name hints and repeating data`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (data, testNameRoot), _ =
            IgnoreBuilder.BuildTestWithSetupDataTestBodyNameHints (testFeature, true)
...
let ``Create a test name with no name hints`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (data, testName), _ =
            IgnoreBuilder.BuildTestWithSetupDataTestBody testFeature
...
let ``Create a test name with no name hints same data repeated`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (_, tests), (data, testName), _ =
            IgnoreBuilder.BuildTestWithSetupDataTestBody (testFeature, true)
...
let ``Not call setup when executed`` =
    feature.Test (fun (_, testFeature: IFeature<string>) ->
        let (monitor, tests), _, _ = IgnoreBuilder.BuildTestWithSetupDataTestBody testFeature
